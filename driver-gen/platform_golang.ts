...
class TempDir {
  ...
  static async createTempDir() {
    ...
    if (code !== 0) throw new Error(`Failed to allocate temp dir: code ${code}`);
  ...
  async generate(tempDir: TempDir) {
      ...
      case "Folder":
        ...
        for (const prop of shape.props) {
          ...
          if (prop.type === "Function") {
            ...
            if (!funct) {
              throw new Error(`Failed to resolve Function ${prop.target} for prop ${prop.name}`);
            }
    ...
    for (const shape of this.shapeNames.values()) {
      if (shape.type !== "Folder") { // TODO
        console.log("WARN: Shape", shape.name, " is not a Folder, not supported yet");
        continue;
      }
      ...
      for (const prop of shape.props) {
        if (prop.type === "Function") {
          ...
          folderWriter.write("  %s *%s\n", snakeToCamel(prop.name), snakeToCamel(prop.type));
        } else {
          folderWriter.write("  %s base.%s\n", snakeToCamel(prop.name), prop.type);
          folderWriter.useDep("base");
        }
      }
      for (const prop of shape['native-props']) {
        folderWriter.write("  %s %s\n", snakeToCamelLower(prop.name), prop.type);
