...
class BlobLiteral {
  ...
  static async fromActual(name, blob) {
  ...
  static fromNodeBuffer(name, buffer, mime=buffer.mime) {
    if (!mime) throw new Error(`BUG: no MIME Type passed with node Buffer`);
    return new BlobLiteral(name, buffer.toString('base64'), mime);
  }
  ...
  async asNodeBuffer() {
    const buffer = new Buffer(this.Data, 'base64');
    buffer.mime = this.Mime;
    return buffer;
  }
...
function inflateSkylinkLiteral(raw) {
  ...
  switch (raw.Type) {
    ...
    case 'Blob':
      if (raw.constructor === BlobLiteral) return raw;
